...
#   MC 1.7+ Version by Oliver Kahrmann (founderio), 2013
    ...
    def handle_read(self):
        ...
        if len(data) > 0 and data[0] == "\n":
            ...
            if (len(self.login) > 0) and (
                    #Multiple signatures from various different versions/modified servers --> in here for historical reasons
                    ...
                    #(self.foundDone and (data.find(" logged in ") != -1)) or
                    (not self.foundDone and (data.find(" Done ") != -1))
                    ):
                while len(self.login) > 0:
                    obj = self.login.pop(0)
                    if not obj.connected and not obj.connecting:
                        log("Attempting ' Done ' connect (%d) with %d remaining in queue" % 
...
class ProxyHandler(asyncore.dispatcher_with_send):
        ...
        if not self.server.connected:
            ...
            if mcProcess == None or mcStopping:
                if data == '':
                    ...
                    packlen,counter = self.read_varint(data, 0)# struct.unpack('>h', data[:2])[0]
                if packlen != 0:
                    c,counter = self.read_varint(data, counter)
                    #c = data[counter]
                    #counter+=1
                    if c == 1: # 'Ping'
                        log("Recieved 'Ping'")
                        ...
                        self.send(data) # 'Ping'
                        log("Sent 'Ping'")
                    elif c == 0: # 'Handshake/Request'
                    	if packlen == 1:
                    	    log("Received Status Request")
                    	    string = '{"description":"' + config['mc']["motd"] + '","players":{"max":' + config['mc']["max-players"] + ',"online":0},"version":{"name":"' + config['ss']["minecraft-version"] + '","protocol":' + config['ss']["protocol-version"] + '}}'
                            
                            strlen = len(string)
                            string = string.encode('utf-8')
                            varlen = strlen + 2
                            
                            desc = chr(varlen) + chr(0) + chr(strlen) + string
                            self.send(desc) # 'Response'
                            log("Sent 'Response: " + string + "'")
                    	else:
                            log("Recieved 'Handshake'")
                            # read out the handshake info
                            cl_protocol_version,counter = self.read_varint(data, counter)
                            strlen,counter = self.read_varint(data, counter)
                            cl_ip = data[counter:(strlen+counter)]
                            counter += strlen
                            cl_port = data[counter:counter+2]
                            counter += 2
                            cl_state,counter = self.read_varint(data, counter)
                            log("cl_protocol_version: " + str(cl_protocol_version))
                            log("cl_ip: " + str(cl_ip))
                            log("cl_port: " + str(cl_port))
                            log("cl_state: " + str(cl_state))
                            
                            if cl_state != 1: # Status
                                # Status (state 1) OK, will be managed when the request (\x01\x00) comes in
                                # for all other states: boot up the server
                                if not mcStopping:
    ...
    def read_varint(self, data, begin):
        result = 0;
        shift = 0;
        c = begin
        while 1:
...
def main():
    ...
    print "MCSuperServer " + str(VER) + " Copyright (C) 2011, 2012  Paul Andreassen\n" \
          "Modifications by Oliver Kahrmann, 2013\n" \
